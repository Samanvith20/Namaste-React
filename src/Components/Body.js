...
const Body = () => {
  ...
  const handleFilterClick = () => {
    const filteredRestaurantsData = restaurantsData.filter(
    ...
    <div className="Body p-4">
      <div className="flex flex-col md:flex-row justify-between items-center mb-4">
        <input
          ...
          className="search-input p-2 w-full md:w-2/3 lg:w-1/2 border border-gray-300 rounded focus:outline-none focus:border-green-500"
        />
        <button
          className="px-4 py-2 ml-2 md:ml-0 md:mr-2 bg-green-500 text-white rounded cursor-pointer transition duration-300 hover:bg-green-600 w-full md:w-auto"
          ...
          <ul className="list-none p-0">
            {suggestions.map((suggestion) => (
              <li className="mb-2" key={suggestion?.info.id}>
                <Link
                  to={`/restaurants/${suggestion?.info.id}`}
                  className="text-black no-underline transition duration-300 hover:text-blue-800"
                >
